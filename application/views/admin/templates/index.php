	...
	<div class="header_title"><a class="separator" href="<?= $this->config->item('domain') ?>">Home</a> <?= ucwords($this->name) ?> List</div>
        <div class='search_panel' style="float: left;">
            ...
            <form action="<?= $this->config->item('admin_url') ?>/<?= $this->name ?>/index" method="get">	
                <?= form_dropdown('status', array(""=>"--All Status--")+$this->config->item('publish_status'), set_value('status',isset($status) ? $status : '' ),' id="status"'); ?>					
...
<div id="submenu">
	<ul>
    	<li><a href="<?php echo $this->config->item('admin_url').'/'.$this->name.'/newd'?>">Add New Template</a></li>
